#!/usr/bin/env janet
# play.janet — cllm Janet binding：基本 ask、串流、schema+JSON(spork/json)、tools、
#   media、modalities、shell(CLI) 呼叫。直接改它就是開發。
# 跑：source ~/dev/cllm/env.sh 後  janet play.janet "$CLLM_FIXTURES"（或 bash run.sh）
#   env.sh 設好 JANET_PATH（含裝好的 llm.so ＋ janet 預設 tree 的 spork）→ (import llm) 直接可用。
(import llm)
(import spork/json)
(import spork/sh)
