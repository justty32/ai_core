...
# tools=[{"name","description","parameters"}], on_tool=fn(call), on_media=fn(m)；錯誤丟 llm.LLMError
```
...
# :schema json → 回 JSON 字串，用 (import spork/json) → (json/decode …) 解
...
// cllm.Schema / cllm.Tools(ToolDef{…}) / cllm.OnTool / cllm.OnMedia / cllm.MediaIn / cllm.Modalities
```
...
-- 完整選項：schema / tools={{name,description,parameters}} / on_tool / on_media / media / modalities
...
; :tools (list (list "name" "desc" "params-json"))；:on-tool (lambda (id name arguments) …)
...
; :tools (list (list :name "…" :description "…" :parameters "…"))；:on-tool (lambda (call) (getf call :name/:arguments))
