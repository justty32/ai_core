...
所以你學一次 API，十種語言都會用。本 lab 每個語言資料夾裡的 `play.*` 就是**同一套示範**（基本 ask ／串流／結構化輸出／tools／media／modalities）用十種語言各寫一遍，**直接改 `play.*` 就是開發**。
...
| 不確定環境是否健康 | `bash ~/repo/ai_core/sub_projs/cllm/core/test/bindings_smoke.sh`（十一語言一鍵，全綠＝正常）|
