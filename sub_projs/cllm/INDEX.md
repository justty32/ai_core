...
| [`bindings/`](bindings/README.md) | **腳本語言綁定**（C ABI 下游消費端）：[lua](bindings/lua/)（`llm.so`，`require("llm")`）＋[s7](bindings/s7/)（`llm-s7` host）＋[python](bindings/python/)（`llm.py`，純 ctypes 零編譯）。API 對齊 galtxt/try_4（`ask`＋`on_delta`）；不進主建置 |
