...
| 旗標 | 說明 |
|------|------|
| `--system <文字>` | system role 指示（**字面文字**，非檔案；在 user 訊息前插一則 system 訊息）。長文用 `--system "$(cat persona.txt)"` |
...
|----|------|--------|
| `0` | 成功 | 正常回答、`--help` |
| `1` | 用法錯 | 未知旗標、缺 prompt（stdin 也空）、「-」但 stdin 是互動終端、`--image`/`--schema`/`--config`/`--tool`/`--modality` 檔讀不到、`--system` 缺值、config／工具定義 JSON 壞、`--media-out` 不是目錄、數值旗標型別錯（如 `--temperature abc`）|
