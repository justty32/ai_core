"""cli.py — 薄 CLI 外殼：把命令列組成一次 llm.ask 的發問（鏡像 C++ 的 `llm` CLI）。

只做「參數解析 + I/O 接線」，真正的活（組請求／打 HTTP／解串流）全丟給 ctypes binding llm.ask
（import llm，over libcllm.so）。周邊拆到姊妹模組（皆對齊 C++ 的分檔，感覺基準＝core-py 的 CLI 側）：
  ...
  解析；media.py＝--media 三分流取值；reqinput.py＝請求類旗標組成 llm.ask 輸入（cli.cpp 組請求段）；
  output.py（cli_output.cpp）＝出口 handlers（Sink）。本檔（對齊 cli.cpp）只留 main 接線。

★ 與 core-py 唯一差別：內核換成 ctypes binding llm.ask（core-py 走純 HTTP 的 cllm.core.ask），
API 形狀一致，故接線邏輯逐字對齊。
...
def main(argv=None):
    ...
    llm.ask(prompt, client.get("endpoint"),
...
def _entry():
    """進入點：包 KeyboardInterrupt → 130（對齊 SIGINT 取消）。"""
    try:
        sys.exit(main())
