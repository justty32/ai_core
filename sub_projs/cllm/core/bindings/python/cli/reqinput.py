"""reqinput.py — 把 CLI 的請求類旗標驗證／組成 llm.ask 的請求輸入（cli.cpp 組請求段）。

--schema/--tool/--modality 的 cfg 收「字面 JSON 文字」（同 --system），要吃檔案內容一律靠 shell
$(cat s.json)。解 JSON 失敗＝用法錯（退 1）。--image/--media 的三分流取值在 media.py
（build_media_item）。本檔把這些旗標收成 llm.ask 要的 schema／tools／modalities／media 四份輸入，
並驗 --media-out 目錄。感覺基準＝core-py 的 cllm/reqinput.py。
"""
import json
import os
import sys
...
class RequestInputs:
    """llm.ask 的請求輸入四件組（schema／tools／modalities／media）。"""
