--[[ flags.lua — 反射旗標表與 --help 用法文字（對齊 core/src/cli_flags.cpp）。

反射旗標＝連線／取樣選項，逐項對應 llm.ask opts 的 snake_case 欄位（見 ../README）；固定旗標
（--stream/--image/--schema/…）的解析在 argv.lua。print_usage 印的清單即由本表生成。 ]]
local internal = require("internal")

local M = {}

-- 反射旗標表：{flag, 欄位名, 型別}。型別 "str"/"int"/"float"。順序／欄位對齊 ask opts。
M.REFLECT_FLAGS = {
...
-- flag → {欄位名, 型別}（argv 掃描時查表）。
M.FLAG_TO_FIELD = {}
for _, e in ipairs(M.REFLECT_FLAGS) do M.FLAG_TO_FIELD[e[1]] = { e[2], e[3] } end
...
function M.print_usage()
  ...
  out:write(
    ...
    "  --help, -h             顯示本說明\n" ..
    "\n連線／取樣旗標（對應 ask opts 欄位，未給即不送、交後端默認）：\n")
  for _, e in ipairs(M.REFLECT_FLAGS) do
    out:write(string.format("  %-22s %s%s\n", e[1], TYPE_HINT[e[3]], FIELD_ANNOT[e[2]] or ""))
  end
  out:write(
