...
| `test/` | `cli_smoke.sh`（離線黑箱煙霧測試，40/40）＋`bindings_smoke.sh`（十一語言綁定一鍵 smoke，輪流呼叫各 `bindings/<lang>/smoke.sh`）＋`fixtures/{fake,fake_stream,fake_tool,fake_json,fake_media}/`（版控的假回應，`file://` 餵進 CLI 與綁定）|
