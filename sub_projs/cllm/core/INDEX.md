...
| [`bindings/`](bindings/README.md) | **九語言綁定 + 常駐開發環境**（C ABI 下游消費端）：C／C++／Lua／Fennel／s7／Python／Janet／Common Lisp／Go 各語言原始碼＋example（含 JSON 解析＋shell-out CLI）；Janet 走 native C 模組（純 FFI 表達不了三參數有回傳的回呼）；Lisp 家族另有 `image/`（產映像／CLI/lib／執行期修改）。[`install-dev.sh`](install-dev.sh) 一鍵裝成常駐前綴 `~/dev`。API 對齊 galtxt/try_4（`ask`＋`on_delta`）；不進主建置 |
...
| `test/` | `cli_smoke.sh`（離線黑箱煙霧測試，31/31）＋`bindings_smoke.sh`（十語言綁定一鍵 smoke，輪流呼叫各 `bindings/<lang>/smoke.sh`）＋`fixtures/{fake,fake_stream,fake_tool,fake_json,fake_media}/`（版控的假回應，`file://` 餵進 CLI 與綁定）|
