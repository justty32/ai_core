...
- **要用某語言呼叫 cllm，或搭常駐 dev 環境** → **[bindings/README.md](bindings/README.md)**（C／C++／Lua／Fennel／s7／Python／Common Lisp／Go 八語言綁定＋Shell CLI）＋ **[install-dev.sh](install-dev.sh)**（一鍵裝到 `~/dev`、可 include/link）。
...
1. **改動程式碼後跑驗證確認行為不變**：快速驗證＝`cmake --build --preset linux-debug && bash test/cli_smoke.sh`（離線黑箱，19/19 綠）。
...
**跨機開發**：Windows（MinGW-w64 g++16＋`C:/dev/vcpkg`）與 Linux（Manjaro，系統 g++16＋`~/vcpkg`）雙機皆已實測全綠。各機路徑釘在 `CMakePresets.json`（`mingw-*` / `linux-*` 兩組 preset，各掛 `condition` 判平台）。完整環境需求／跨平台細節／換機改哪幾處在 [docs/setup.md](docs/setup.md)（環境＋vcpkg 依賴）＋ [docs/platform.md](docs/platform.md)（跨平台）。**常駐開發環境**（把 cllm 裝成 `~/dev` 的可 include/link 前綴＋八語言環境、可重現；⚠ `~/dev` 是共用目錄、別整個 rm）走 [install-dev.sh](install-dev.sh)。
