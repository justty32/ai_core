...
離線 fixture（`test/fixtures/{fake,fake_stream,fake_tool,fake_json,fake_media}/`）＋ `test/cli_smoke.sh` 端到端驗 `llm` CLI（31/31 綠）＋ `test/bindings_smoke.sh` 一鍵驗九語言綁定（輪流呼叫各 `bindings/<lang>/smoke.sh`）。
...
**② `llm` CLI（unix filter，`src/cli.*`＋`src/main.cpp`）**：走 C++ 鏡像 `cabi.hpp` 消費 `libcllm`。prompt 走位置參數、沒給就讀 stdin；答案吐 stdout、診斷吐 stderr。**連線／取樣旗標從 `llm::abi::Client` 欄位反射生成**（`glz::for_each_field`＋`reflect::keys`；加欄位＝旗標與 `--help` 自動長出來），固定旗標（`--stream`／`--image`／`--schema`／`--config`／`--help`）手寫。設定三層來源：內建預設 → config 檔 → 命令列旗標。
