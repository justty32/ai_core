...
| [core-py/](core-py/) | **cllm 的純 Python 平行實作**——與 `core/` 並列，做同一件事但**不碰 C ABI**、用標準庫 `urllib` 直接打 OpenAI 相容 HTTP。可 import 的 `cllm` 內核＋薄 CLI 外殼（旗標鏡像 `llm`）。零外部相依。 | [README.md](core-py/README.md) |
