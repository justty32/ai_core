    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        with open(cfg_path, "w", encoding="utf-8") as f:
            json.dump({"endpoint": "http://x/v1/chat/completions", "model": "m",
                       "timeout_ms": 120000}, f)
        store.patch_cllm_api_key("BEARER123")
        ...
        check(got["endpoint"] == "http://x/v1/chat/completions" and got["model"] == "m"
              and got["timeout_ms"] == 120000, "其他 cllm 設定鍵原樣保留")


def main():
    for t in (test_pkce, test_authorize_url, test_token_store_roundtrip,
              test_is_expired, test_patch_config_preserves_keys):
        print(t.__name__)
        t()
    print("\n全部離線煙霧測試通過。")
