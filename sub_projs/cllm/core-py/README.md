...
│   ├── core.py       # ★ 內核門面（cabi.cpp）：ask 出口＋接線，分派給下列關注點模組
│   ├── errors.py     # 內核葉：LLMError／DEFAULT_ENDPOINT（被 request/http/response/stream 共用）
│   ├── request.py    # 組請求 body＝唯一請求真相源（cabi_request.cpp）
│   ├── http.py       # 傳輸管子：file:// 與真 HTTP（http.cpp）
│   ├── response.py   # 非串流解析＋錯誤護欄（cabi_response.cpp）
│   ├── stream.py     # SSE 串流解析（cabi_stream.cpp）
│   ├── cli.py        # 薄 CLI 外殼（cli.cpp）：main 接線、_entry
│   ├── argv.py       # argv 掃描（cli.cpp 解析段）→ ParsedArgs
│   ├── reqinput.py   # 請求類旗標組成 core.ask 輸入（cli.cpp 組請求段）
│   ├── internal.py   # CLI 葉（cli_internal.hpp）：退出碼／env 鍵／檔案讀取
...
├── test/
│   ├── cli_smoke.py         # 離線黑箱煙霧測試入口（走 file:// 假回應，仿 cli_smoke.sh）
│   ├── smoke_harness.py     # 測試共用底：斷言／計數／跑 CLI／暫存檔清理
│   ├── smoke_cases_output.py # case：輸出／body 正確（happy path、system、tools、media 直通）
│   └── smoke_cases_exit.py  # case：設定來源＋退出碼分流
├── pyproject.toml
└── README.md
```

**套件名選 `cllm`**（package）。內核門面在 `cllm/core.py`，按關注點拆到 `errors`/`request`/
`http`/`response`/`stream`（括號內為對齊的 C++ 檔）。CLI 外殼 `cllm/cli.py` 只留 main 接線，
周邊拆到姊妹模組（`argv`/`reqinput`/`internal`/`flags`/`config`/`media`/`output`）。
每檔單一職責、≤150 行、依賴為無環 DAG（`errors`/`internal` 為葉）。
