...
#
# 這兩支是 C++ 二進位，vendored 的是「編好的產物」而非原始碼：從 $CLLM_ROOT/build/tools 複製
# anthropic-proxy + llm-login + liblogin.so 進 vendor/cllm-tools/，之後 up.sh 就能離線、與 repo
# 解耦地起 sidecar。上游更新時重跑本腳本即可。
#
# ⚠ 沿用姊妹專案 janet-try-1 的同名腳本（tools 產物與兩專案共用，不重跑 cmake）。
#
...
if [ ! -d "$SRC" ]; then
  ...
  echo "（tools 已由姊妹 agent 建好；若真的缺，才 cd $CLLM_ROOT && cmake --build --preset linux-debug）" >&2
  exit 1
fi
