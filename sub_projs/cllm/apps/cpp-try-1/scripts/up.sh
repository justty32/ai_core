#!/usr/bin/env bash
# up.sh —— 一鍵把整條佈線拉起來再跑 C++ app（INTEGRATION.md「情境 B / up 動作」）。
#
#   cpp app  ──OpenAI+Bearer──▶  anthropic-proxy(sidecar)  ──x-api-key──▶  api.anthropic.com
...
# （llm-login 或 sk-ant key）③ 帶 APP_ENDPOINT/APP_API_KEY/APP_MODEL 進來跑 C++ app。
#
...
APP_BIN="$HERE/build/cpp-try-1"

# ── 確保 app 已編 ───────────────────────────────────────────────
if [ ! -x "$APP_BIN" ]; then
  echo "· app 尚未編，跑 build.sh …"
  "$HERE/build.sh"
fi
...
start_proxy() {
  ...
  if [ -z "$bin" ]; then
    echo "✗ 找不到 anthropic-proxy —— 先 vendor 或用 build 產物：" >&2
    echo "    ./scripts/vendor.sh" >&2
    echo "    （tools 已由姊妹 agent 建好，通常在 $CLLM_ROOT/build/tools/）" >&2
    exit 1
  fi
