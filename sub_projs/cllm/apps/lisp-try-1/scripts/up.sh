#!/usr/bin/env bash
# up.sh —— 一鍵把整條佈線拉起來再跑 lisp app（INTEGRATION.md「up 動作 / vendored sidecar」）。
#
#   lisp app  ──OpenAI+Bearer──▶  anthropic-proxy(sidecar)  ──x-api-key──▶  api.anthropic.com
#   src/main.lisp                 127.0.0.1:8787              (翻 wire format)
#   憑證：貼 sk-ant key（MODE=anthropic）或 llm-login OAuth 帳號登入（MODE=openrouter）
#
# 兩種模式（MODE 環境變數，與 janet-try-1 一致）：
...
# （sk-ant key 或 llm-login token）③ 帶 APP_ENDPOINT/APP_API_KEY/APP_MODEL 進來跑 lisp app。
#
...
# 掛上 cllm 開發環境（CLLM_LISP / LIBCLLM / PATH…），好讓 lisp app 執行期載得到 binding
...
start_proxy() {
  ...
  echo "· proxy health-check 通過（TCP 127.0.0.1:$PORT；connection refused 這類就不會發生了）"
}
...
case "$MODE" in
  anthropic)
    ...
    if [ -z "$key" ]; then
      ...
      echo "    → 或 export ANTHROPIC_API_KEY=sk-ant-...；或（走 OAuth）用 MODE=openrouter" >&2
    fi
  ...
  openrouter)
    key="$(try_login_token)"
    if [ -z "$key" ]; then
      echo "✗ OpenRouter OAuth 尚無有效 token。先做帳號登入（開瀏覽器一次，屬 WAIT_USER）：" >&2
...
# 交給 run.sh 的 run 子命令跑 CLI（APP_* 已 export，會被子行程繼承）
exec "$HERE/scripts/run.sh" run "$@"
