...
- **Manjaro 家機首跑驗證 try_2 跨平台**：程式已按跨平台寫法（`build.sh` uname 偵測、`_path.lua`/`llm.lua` OS 分流、`host.cpp` `#ifdef _WIN32`），**但只在 Windows 實測過**。回家在 Manjaro 首跑：`cd try_2 && ./build.sh && ./build/host.exe test/test.lua`，確認離線全綠（尤其 `pwd` cwd 探測、`/tmp` 暫存、`file://` URL、原生 UTF-8 argv）。有壞再回報。
