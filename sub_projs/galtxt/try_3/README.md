...
client.ask("你好", true, [](std::string_view delta){ …; return false; });  // 串流（回 true 中止）
```
...
- ★ **後端錯誤會 throw、不靜默回空**（護欄，2026-07-15 try_4 真後端驗揪出並修在此）：OpenAI 相容後端出錯（LM Studio 模型載入失敗、雲端 401/429…）回 `{"error":{"message":…}}`＋HTTP 4xx、**無 `choices`**。原本「`choices` 空就 `return {}`」會把後端錯誤吞成空字串還報成功——呼叫端分不清「模型回空」與「後端出錯」。現由 `llm.cpp` 的 `throw_if_backend_error(status, raw)`（解 error 物件 throw、或 HTTP≥400 帶狀態碼＋body 片段 throw）在**共用 `post()` 一處**攔——四個非串流入口（`ask`/`ask_json`/`ask_tools`/`ask_vision`）全數跟上，串流路徑另接 `http::stream` 的 status＋累積 raw 再攔一次。**離線 `file://` 回 status=200 且無 error 物件，不受影響**（10 支離線 fixture 不回歸）。⚠ 又一個「離線 fixture 從不回錯、只投影成功」故驗不出的缺陷（見 [common/gotchas](../workflows/common/gotchas.md) 方法論條 ③）。
...
> ★ 2026-07 C ABI 重構後的第二個交付物。**此段取代**上面「上層 LLM 接口」描述的舊世界——
> 那批可獨立呼叫的 L0 C++ 入口（`llm::Client`／`ask`／`ask_tools`／`ask_vision`／`ask_as`）連同
> `from_env()`／`--real` demo 一起**封存進 `archived/`**，`src/` 只留對外 C ABI（`cabi.*`）＋傳輸管子
> `http.*`。CLI 是 `libgaltxt.so` 之上的薄消費端，走 **C++ 鏡像 `cabi.hpp`（`llm::abi::Client`）**，
> 唯一入口 `llm_ask` 統一吃 prompt＋schema＋media。
