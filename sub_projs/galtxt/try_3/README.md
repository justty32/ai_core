...
> 1. **`libgaltxt.so` —— 對外 C ABI**（`src/cabi.*`，`extern "C"`）：唯一入口 `llm_ask` 統一吃 prompt＋
>    schema＋tools＋media＋modalities＋stream。純 C 客戶端 include `cabi.h`、連 `-lgaltxt` 即用；另附
>    header-only 的 C++ 薄鏡像 `cabi.hpp`（`llm::abi`）。
> 2. **`llm` —— unix filter CLI**（`src/cli.*`＋`src/main.cpp`）：走 `cabi.hpp` 消費 `libgaltxt`。
...
> **融進 C ABI 實作**（見「對外 C ABI」一節），不再對外提供獨立函數。**try_4 那條線仍借編這批舊 L0** 的 C++
> function API（見 [try_4](../try_4/README.md)），故封存而非刪除。
...
cmake --build --preset mingw-debug  # 建置 → build/libgaltxt.so ＋ build/llm.exe
./build/llm.exe 你好                # 跑 CLI（Windows 產物帶 .exe）

# Linux（linux-debug／linux-release，指令完全同構）
cmake --preset linux-debug
cmake --build --preset linux-debug  # 建置 → build/libgaltxt.so ＋ build/llm
...
> ⚠ **CLI 產物名跨平台不同**：`.so` 兩邊都叫 `libgaltxt.so`；`llm` 執行檔在 Windows 是 `llm.exe`、Linux 是
...
3. `CMakeLists.txt`：`find_package(glaze CONFIG REQUIRED)` ＋ `target_link_libraries(galtxt PRIVATE glaze::glaze)`（`llm` CLI 另連 `galtxt` 拿 `llm_ask`＋`glaze` 做反射）。glaze 需 **C++23**（已把 `CMAKE_CXX_STANDARD` 提到 23，g++16 支援）。
...
- **fixture 路徑不寫死**：`llm` CLI 的 `--endpoint file://<絕對路徑>` 就是反射欄位，`test/cli_smoke.sh` 在**執行期**用 `$ROOT` 組出 `file://` 路徑餵進去（比照 try_2 `_path.lua` 不綁機器路徑的精神）——比舊 demo exe 靠 `TRY3_SOURCE_DIR` 編譯期注入更活。（try_4 那條線另借編 http，靠 `TRY4_TRY3_DIR` 注入 try_3 目錄讀同一批 fixture。）
...
## 對外 C ABI（`libgaltxt.so`，`src/cabi.*`）
...
```c
#include "cabi.h"   // 純 C 客戶端；連 -lgaltxt
...
> `libgaltxt`，唯一入口 `llm_ask` 統一吃 prompt＋schema＋media。它也是那薄鏡像的第一個真實消費端——
> 反射生成旗標這一手，正是把 try_1/try_2「不搬 schema 表」的主張延伸到 CLI 那一哩（改反射新的 `llm::abi::Client`）。
...
- ⚠ **真後端的行為（含錯誤路徑）驗證由 try_4 那條線扛**（它借編這批舊 L0＋掛 s7／Lua，見 [try_4](../try_4/README.md) 的「里程碑 3」）；上面「對外 C ABI」的後端錯誤護欄就是那次真後端驗揪出來、修在 `guard_backend_error` 的。`llm` CLI 走的是同一套 `http`＋請求／回應解析，行為一致。
...
cmake --build --preset linux-debug  # 7 個編譯單元全過（.so 5：cabi.cpp/cabi_request/response/stream＋http；
                                     #   CLI 2：cli.cpp＋main.cpp），link 出 build/libgaltxt.so ＋ build/llm
...
**產物名**：`.so` 兩邊都叫 `libgaltxt.so`。`llm` 執行檔在 Linux **不加副檔名**（`build/llm`）、Windows 是 `build/llm.exe`——CLI 是 unix filter，Linux 側刻意讓它就叫 `llm`。（對照：舊 demo exe 曾為「指令跨平台一致」硬掛 `set_target_properties(... SUFFIX ".exe")` 讓 Linux 也叫 `try3.exe`；那顆 exe 連同該約定已封存，新的 `llm` target 不再沿用。）
