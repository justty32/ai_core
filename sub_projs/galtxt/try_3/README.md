...
目前狀態：**可建置、可 VSCode/gdb 除錯，兩塊地基＋上層 LLM 接口都長出來了；Windows／Linux（Manjaro）雙機皆已實測**。原始碼：`src/demo.{hpp,cpp}`（`sum_to`/`greet` 除錯示範）、`src/http.{hpp,cpp}`（native HTTP 傳輸）、`src/llm.{hpp,cpp}`（`llm::Client` ask 接口）、`src/llm_tool.{hpp,cpp}`（工具呼叫）、`src/llm_media.{hpp,cpp}`（多媒體/vision）、`src/llm_json.{hpp,cpp}`（結構化輸出）。`build/try3.exe` 一趟把全部離線 fixture 跑一遍：計算/字串、glaze 往返、HTTP file:// GET＋串流、`ask` 非串流/串流、工具呼叫、vision、結構化輸出，全綠——Windows 用 mingw preset、Linux 用 linux preset，兩邊產物都叫 `try3.exe`（詳見下方「跨平台」一節）。
