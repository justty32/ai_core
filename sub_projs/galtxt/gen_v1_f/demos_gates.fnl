;; demos_gates.fnl — 示範⑤～⑨：執行期寫入門／洩底／排斥邊／speculate／約束細化。全數自帶 assert。
...
(fn run [db]
  ...
  (let [tx (db:begin)]
    ...
    (let [(pass why) (tx.check)]
      (assert (and (not pass) (why:find "矛盾防線" 1 true)) "⑤ 他引用真相邊應被矛盾防線擋下"))
    (tx.rollback))
  ...
  (let [tx (db:begin)]
    ...
    (assert (tx.check))
    (tx.commit))
  (assert (and (. db.by_id "物#書417" :canon) (. db.by_id "邊#誤會" :canon)) "⑤ 呈現過即 canon")

  (let [(ok) (pcall #(db:modify "物#書417" "普通文庫"))]
  ...
  (assert (= (length (db:match { :who "柏木秋穗" :state "hides" })) 0)
          "⑥ 洩底後 hides 清空（恰一記帳可據此計數）")
  (assert (= (db:resolve "邊#她心動" (db:view "佐伯悠")) "她對他心動")
  ...
  (let [tx (db:begin)]
    ...
    (let [(pass why) (tx.check)]
      (assert (and (not pass) (why:find "排斥邊" 1 true)) "⑦ 在架已 canon，已售不得再呈現"))
    (tx.rollback))
  ...
  (let [m1 (table.concat (db:match { :kind "連結" }) ";")
        m2 (table.concat (db:match { :kind "連結" }) ";")]
    (assert (and (= m1 m2) (not= m1 "")) "⑧ match 恆為插入序（確定性）"))
  (print "⑧ speculate 分支不落盤＋查詢確定性　OK")

  ;;; -------------------- ⑨ 約束細化：解不出→掛起工單（編譯期 LLM 崗位）
  (let [(got ticket) (db:refine "場#校園後方" { :grain "場所級" :constraints { :mood "櫻花" } })]
    (assert (and (= got nil) (ticket:find "掛起工單" 1 true)) "⑨ 庫裡沒有滿足點→掛起"))
  (assert (= (length db.pending) 1))

  ;; 編譯期補件（LLM 炸候選→人審→入庫的入庫那一步；分佈未鎖，世界為故事讓路）
  (db:add { :id "場#櫻花樹下" :kind "節點" :parent "場#校園後方"
            :mood "櫻花" :value "校園後方・櫻花樹下" })
  (let [got (db:refine "場#校園後方" { :grain "場所級" :constraints { :mood "櫻花" } })]
