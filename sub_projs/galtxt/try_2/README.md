...
除錯器兩邊都是 **Local Lua Debugger**（`tomblind/local-lua-debugger-vscode`，純 Lua、不需原生模組）；
`.vscode/launch.json` 那六個設定**兩個編輯器共用**（nvim-dap 會自動讀 `.vscode/launch.json`）。

### ⚠ 先讀：Lua 5.5 讓這個除錯器直接壞掉（現況＝**nvim 可用、VSCode 不可用**）

**本專案 vendored 的是 Lua 5.5，而 `local-lua-debugger-vscode`（0.3.3）還沒跟上。** 它注入被除錯行程的
`lldebugger.lua` 裡有這一段：
...
（`attempt to assign to const variable 'path'`）——整個 `lldebugger` 模組載入不了，除錯器根本沒起來。

> **症狀極度誤導**：DAP session 起得來、中斷點還顯示 `verified=true`，然後程式就直接跑完結束、什麼都沒停。
> 看起來像「中斷點沒作用」。真正的錯誤只出現在 DAP 的 `output/stderr` 事件裡。

**現況（2026-07-14，使用者拍板）**：
- **Neovim ＝ 可用**。修補做在使用者的 nvim 設定 repo（`lua/plugins/dap.lua`）裡，是**啟動時的冪等
  patch**——因為檔案在 mason 的套件目錄下，mason 更新會覆蓋回去。偵測到壞的樣子才改；已修好、
  或上游哪天修了，就是 no-op。
- **VSCode ＝ 已知不可用，且刻意不修**。VSCode 沒地方掛這種啟動 hook，**Lua 除錯一律走 nvim**。
  真要在 VSCode 修的話，手動改這個檔的那一行（換個變數名即可）：
  ```
  ~/.vscode/extensions/tomblind.local-lua-debugger-vscode-*/debugger/lldebugger.lua
  ```
  ```lua
  for rawPath in scriptRootsStr:gmatch("[^;]+") do
      local path = Path.format(rawPath) .. Path.separator
  ```
  ⚠ 擴充套件更新會覆蓋回去，得重修。（上游修好之前都是這樣。）

### 步驟

1. 裝擴充套件 **Local Lua Debugger**（發布者 `tomblind`，ID `tomblind.local-lua-debugger-vscode`）——純 Lua 除錯器、不需原生模組。（另可裝 `sumneko.lua` 做 IntelliSense。）nvim 側走 mason 裝同一個套件。
