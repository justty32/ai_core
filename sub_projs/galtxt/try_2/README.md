...
├─ native/cjson.c       ★ native（C）JSON codec，編進 liblua.a → 內建 require("cjson")
...
build/host.exe (C++)  ── 內嵌 Lua 5.5 VM、綁命令列成全域 arg 表、UTF-8、load 並跑 .lua 腳本
   │  跑                （liblua.a 內含 native/cjson.c；stock lua.exe 亦同源、同樣內建 cjson）
   ├─ src/cli.lua ── 由 llm.lua 的 schema「生成」--flag 介面：解析 arg → 呼叫 llm_entry
   ├─ src/llm.lua ── ★ 接口本體 + 參數 schema（唯一真相源）；組請求→curl→解析回應
   │     └─ require ─▶ cjson（native C，編進 liblua.a、經 linit.c 註冊）；encode/decode
   └─ test/test.lua ── 全離線 E2E（curl file:// 灌 test/fixtures/ 假回應）
```

> 模組間用 `debug.getinfo` **自解腳本位址**載入 sibling（location-independent，不綁 CWD）；
> fixture 路徑由 `src/_path.lua` 從專案根算出（不寫死機器路徑）；請求暫存寫到系統 `%TEMP%`。
...
這條縫**背後的實作正在逐步下沉到 C**：JSON 組解已是 native `cjson`（[native/cjson.c](native/cjson.c)，取代原 rxi/json.lua）；
HTTP 傳輸目前仍走 Lua 的 `io.popen("curl …")`。下一步可把傳輸也收進 C 原生函式（`lua_newtable`/`lua_setfield` 直接推 table），
而呼叫端 `llm.ask{…}` 一行都不用改——這正是「縫的價值」：實作換血、契約不動。
...
`test.lua` 把 `base_url` 指到 `file://…/test/fixtures/fake`，curl 讀 [`test/fixtures/fake/chat/completions`](test/fixtures/fake/) 假回應，
...
| 系統 `%TEMP%/galtxt_llm_req.json` | 請求 body 暫存（不在專案內）| ✗ |
