...
├─ vendor/lua/          vendored Lua 5.5.0 原始碼
...
build/host.exe (C++)  ── 內嵌 Lua 5.5 VM、綁命令列成全域 arg 表、UTF-8、load 並跑 .lua 腳本
   ...
   │     └─ dofile ─▶ src/json.lua ── vendored rxi/json.lua 0.1.2（MIT）；encode/decode
...
2. `./build.sh luaexe` 產生 `build/lua.exe`（除錯器要標準直譯器；**不能用 host.exe**——它不吃除錯器注入的 `-e` 引導）。
3. VSCode「File > Open Folder」開 **try_2** 這個資料夾（讓 `${workspaceFolder}` 指到這裡）。
4. 在 `src/cli.lua`／`src/llm.lua`／`src/json.lua` 想停的行按左側下中斷點 → F5，選設定檔（[.vscode/launch.json](.vscode/launch.json) 已備好離線／真後端／目前檔三種）→ 單步、看變數、call stack。
...
| `src/json.lua` | vendored rxi/json.lua 0.1.2（MIT）；encode/decode | ✓ |
