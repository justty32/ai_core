...
return {
  ...
  -- 這一場戲想寫什麼（呼叫方的「劇本意圖」）。
  -- 兩種寫法：點座標（低階 API）或只給 intent（高階 API，座標交給搜尋）。
  scene = {
    coordinate = {
      topic  = "A1",   -- 話題入口（02 軸表：A1 推理文庫）；nil＝交給搜尋
      give   = "B1",   -- 誰跨半步（B1 她讓渡）；nil＝交給搜尋
      gap    = "C2",   -- 反差格（C2 口頭禪遮掩）；nil＝交給搜尋
      ending = nil,    -- 收尾：nil ＝ D≈f(A) 派生（有 intent 時改為全枚舉搜尋）
    },
    intent  = nil,     -- 高階意圖："甜" / "靜" / nil——留空的座標軸依它選
