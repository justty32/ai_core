...
新增 / 修改 galtxt 功能的工作流。這是本工作流的**入口**：先讀本檔，再往下深入。always-on 鐵律見 [AGENTS.md](../../AGENTS.md)；要整理結構時參考 [DEV-GUIDE](../../DEV-GUIDE.md)（被動）；**程式碼慣例 + 導航 index 維護鏈**見 [common/conventions](../common/conventions.md)。

> **現況：尚未開工**（galtxt 剛開、無程式碼）。本檔是「資料夾型工作流」的範例骨架、留檔待用；第一片程式碼開工時再把流程改成實況。`landed/`、`gotchas.md`、`session-log.md` 都是**長出來才建**（見 DEV-GUIDE 四級成長軌跡），不要預先建空檔。

## 流程

```
修改（增量）
  → 跑自動驗證（測試 / build / lint 指令，見 [testing](../testing.md)；目前尚無）綠燈
