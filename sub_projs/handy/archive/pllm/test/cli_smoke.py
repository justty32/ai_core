#!/usr/bin/env python3
"""cli_smoke.py — pllm CLI 的離線黑箱煙霧測試（仿 core/test/cli_smoke.sh）。

精神同 shell 版：CLI 是個可跑的程式，最誠實的測法就是「真的用 subprocess 跑它、驗 stdout／
退出碼」。全程走 file:// 假回應——用自帶的 test/fixtures，不連網、不吃真後端。跨平台
（Windows + POSIX）：file:// URL 由 fixtures 絕對路徑（正斜線）拼成，內核的 _file_url_to_path
兩平台都解得動。

用：`python test/cli_smoke.py`（無需先安裝；用 -m pllm 跑，靠 PYTHONPATH 找到 package）。
