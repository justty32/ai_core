# pllm — handy 的 LLM 地基（cllm 的純 Python 版，可 import／可 pip install）

← [handy](../)（重建中）

這是 **handy 的共用 LLM 地基**：一個零外部相依的純 Python 套件，把 prompt 組成 OpenAI 相容的
`/chat/completions` 請求、打出去、解回應。**未來 handy 的住戶都靠它當「腦」**——不再各自手搓 HTTP、
不再 shell-out 到 C++ `llm` CLI，而是 `from pllm import ask` 直接呼叫。

命名：**p**ython **llm** ＝ `pllm`，與 cllm（C++ 版）平行、名字不撞。

- **當函式庫**：`from pllm import ask` → `ask("你好")`，`tools`／`media`／`stream`／`schema` 全是 kwargs。
- **當 CLI**：`python -m pllm 你好` 或裝成 `pllm` 指令；旗標鏡像 cllm 的 `llm` CLI。
...
> （請求／回應欄位對齊 C++ `core/`）；本副本是 handy 自包含的 vendored 版、更名為 `pllm`（連 fixtures
> 一起帶入，故無 `../core` 外部相依）。要改請求／回應語意，先動源頭再同步過來。
...
```sh
# 在住戶自己的環境（或專案 venv）裡：
pip install -e /home/lorkhan/repo/ai_core/sub_projs/handy/pllm
# 或用相對路徑（從住戶資料夾往上指）：
pip install -e ../pllm
```

裝完就能 `import pllm`，並多出 `pllm` 指令。

**不裝也能用**：把本資料夾加進 `PYTHONPATH` 即可（`PYTHONPATH=/…/pllm python -c "import pllm"`），
CLI 則 `python -m pllm …`（在本資料夾內）。
...
`tools`／`media`／`modalities` 皆 kwargs、可任意組合。完整簽章與陷阱見 `pllm/core.py` 的模組 docstring。
...
```sh
python -m pllm 用一句話介紹你自己
cat report.txt | python -m pllm 總結這份            # prompt＋stdin 合體
git diff | python -m pllm 把 - 寫成 commit 訊息      # 「-」明指 stdin 插入點
python -m pllm --stream -- --開頭的-prompt           # -- 之後一律當 prompt
python -m pllm --help                                # 完整旗標說明
```
...
- **`--schema` / `--tool` / `--modality`：收「字面 JSON 文字」，不讀檔**（同 `--system`）。
  要吃檔案內容一律 `$(cat …)`：
  ```sh
  python -m pllm 給我角色 --schema '{"type":"object","required":["name"]}'
  python -m pllm 給我角色 --schema "$(cat schema.json)"
  python -m pllm 說你好 --modality 'audio={"voice":"alloy"}'   # 名=<字面JSON>；只給 audio＝無參數
  ```
...
手動指向假回應：`python -m pllm 你好 --endpoint "file://$(pwd)/test/fixtures/fake/chat/completions"`。

## 結構

```
pllm/
├── pllm/            # 套件本體（import 名＝pllm）
...
├── pyproject.toml   # 封裝（name=pllm、script: pllm）
└── README.md
```
