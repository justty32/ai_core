...
| `ports/cl/` | `./hermy …` | SBCL `--script` 純內建、中文不亂碼；**CLI 薄殼各 5 行**（邏輯全進 `src/util.lisp` 的 `package hermy`，庫 195 行）——CLI 只「載入庫＋呼叫一個 main」 |
| `ports/fennel/` | `./hermy …` | Fennel（on Lua，handy 本命）；同 CL 的**薄殼＋庫**——**CLI 各 6 行**，邏輯全進 `src/util.fnl`（庫 203 行）；`require`＋vararg 自路徑定位 |
| `ports/s7/` | `./hermy …`（需 `llm-s7`＋`env.sh`）| s7 Scheme（跑在 cllm 的 `llm-s7`）；s7 定位不到自身路徑 → **CLI 各 6 行 shell shim**（算根＋exec llm-s7）＋`src/util.scm` 庫（288 行）；curl+jq，未造輪子 |
