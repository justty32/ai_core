...
```sh
pip install litellm                  # ⚠ 唯一外部相依（當函式庫 import 時要自己備好）
python util/llm/test/smoke.py        # 離線冒煙（不連網、不需 litellm）；現況 49/49
```

> 直接跑住戶（`./llme.py …`）不必先裝：住戶檔頭是 `#!/usr/bin/env -S uv run --script` ＋ PEP 723
> inline metadata，**依賴寫在檔案自己身上**，uv 會自己備環境。兩台開發機的差異與 `UV_PYTHON` 坑見
> [SESSION-LOG](SESSION-LOG.md#開發環境本機-durable-注記--兩台輪流)。
...
上 PATH 當裸命令：`ln -sf "$(pwd)/llme.py" ~/.local/bin/llme`（shebang ＝ `uv run --script`，依賴自帶，直接可執行）。

**刻意極簡**：一個主函式，就是「讀 json → 查 endpoint → 填旗標 → 併 argv → 轉發」。
...
**不必回頭改 llme.py**。慣用欄位：`endpoint`／`model`／`timeout_ms`／`api_key`，取樣預設
（`temperature`、`max_tokens`…）也直接寫在各 endpoint 上；值寫 `null`＝不送（交後端預設）。
...
**命令列自帶的旗標一律贏過設定檔**：llme 組旗標前會掃一遍透傳參數，使用者已給的
（如 `--temperature`、`--api-key`）設定檔同名欄位就直接不送。`--` 之後照 `util.llm` 慣例
一律當 prompt、不參與判定。

### api_key 來源

金鑰**只有一個來源**：`llme.json` 該 endpoint 的 `api_key`（通常寫成 `{"$env":"…"}`，由 `util.config`
從環境變數帶入）。本地免 key 的 endpoint 不填即可；命令列 `--api-key` 依上節規則優先。
