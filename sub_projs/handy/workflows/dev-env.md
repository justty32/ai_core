...
本專案吃現成、別憑記憶重建（現成件清單見 [INDEX「不重造」](../INDEX.md)）。建 cllm 見 [cllm/AGENTS.md](../../cllm/AGENTS.md)（CMake＋Ninja＋vcpkg）。`llme`/`zhtw` runtime 只需 `llm`（cllm 的 CLI）＋ `fennel` 在 PATH（上 PATH 法見 [llme/README.md](../llme/README.md)）。
