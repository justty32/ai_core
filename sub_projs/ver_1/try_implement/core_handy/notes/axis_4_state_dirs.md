...
> ⚠️ 歷史討論記錄（2026-06-28 收斂）。軸 4 是純設施軸，**事實基準在 `../impl/state.hpp`**（描述面外包給軸 3，見 `defs/axes.hpp`）。本檔只留 Round 2 D-API 拍板與現況；已過時的 Round 1 描述性提案見 [`archived/axis_4_history.md`](archived/axis_4_history.md)。
