#!/usr/bin/env bash
# up.sh —— 一鍵把整條佈線拉起來再跑 Lua app（INTEGRATION.md「情境 B / up 動作」）。
#
#   lua app  ──OpenAI+Bearer──▶  anthropic-proxy(sidecar)  ──x-api-key──▶  api.anthropic.com
...
#
# 做四件事：① 掛上 cllm Lua 環境（LUA_CPATH_5_4／5_5，讓 require("llm") 找得到 llm.so）
# ② 確保 sidecar 在跑（僅 anthropic 模式，TCP health-check）③ 確保有憑證（llm-login 或 sk-ant key）
# ④ 帶 APP_ENDPOINT/APP_API_KEY/APP_MODEL 進來跑 Lua app。
#
...
CLLM_ENV="${CLLM_ENV:-$HOME/dev/cllm/env.sh}"
RUNDIR="$HERE/.run"; mkdir -p "$RUNDIR"
PIDFILE="$RUNDIR/proxy.pid"; LOGFILE="$RUNDIR/proxy.log"

# ── 掛上 cllm Lua 環境：require("llm")／require("dkjson") 靠 LUA_CPATH_5_x／LUA_PATH_5_x ──
if [ -z "${LUA_CPATH_5_4:-}" ] && [ -z "${LUA_CPATH_5_5:-}" ] && [ -f "$CLLM_ENV" ]; then
