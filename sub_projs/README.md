...
| [cllm-apps/](cllm-apps/) | **handy 四工具多語言移植（原 `~/code/cllm-apps`，2026-07-19 納入）**：把 [handy](handy/AGENTS.md) 的 `llme`/`zhtw`/`wf`/`mail` 以 **shell-out 方式**移到各語言（真正的「手」仍轉呼 `llm`/`claude`/同語言 sibling，不用 cllm binding），行為 1:1 對齊 Fennel 原型。完成品 `python-handy/`／`cpp-handy/`／`lisp-handy/`（＋規劃中 `janet-handy/`）；早期探索 `*-try-1/`（cpp/janet/lisp/lua）。每 app 附 `configs/`（deepseek keyless＋anthropic-proxy）＋`scripts/{vendor.sh,up.sh}` 串接 cllm 周邊（anthropic-proxy／llm-login）。契約權威＝`HANDY-PORT-SPEC.md`。`build/`／`vendor/`／`.run/` 自帶 `.gitignore` 不進版控、可 regen。 | [HANDY-PORT-SPEC.md](cllm-apps/HANDY-PORT-SPEC.md) |
