...
## 做到哪：四刀進度（全綠、全 commit）

| 刀 | 是什麼 | 關鍵驗證 | 檔案 |
|---|---|---|---|
| **step 1** | 假靜態樹，證 FUSE mount 管線 | `ls`/`cat`/`echo` walk/讀/寫皆通 | `unipath_fs.py` |
| **step 2** | 暴露**自身** process 執行態 | counter 隨時間變（非快照）、`echo` write-through、`ctl` | `unipath_live.py` |
| **step 3** | **跨 process** 暴露 | 從外部 walk 另一 process 的 env、獨立第三方確認改到發布端本身 | `unipath_env.py` + `unipath_pub.py` + `unipath_mount.py` |
| **step 4** | **真 9P2000 線格式** | 獨立真 9P client 自測互通；可被核心 `v9fs` 掛載 | `unipath_9p.py` |

**里程碑**：step 3 做完「歸一於路徑第二類」（正在執行的東西、從外部定址）；step 4 坐實「規範＝講 9P」，並帶來**網路透明＝跨機器＝兩世界通聯**的使能件。
...
step 4（真 9P）:
   [發布端 process]                       [核心 v9fs]（或獨立 9P client）
   活 env + tick                          mount -t 9p
   監聽 TCP:5640  <──── 真 9P2000 線格式 ────  ls/cat/echo（走核心，脫離 FUSE）
```

- `unipath_env.py` ＝執行態環境的**純邏輯**（無 FUSE/socket 依賴），被 step 3/4 共用。
...
- **C 接 tick**：把 `tick(env, 影響) → 下個狀態` 落成真回合制轉移（現在只有背景 `+1` 雛形）。
- **D 往階段二**：在這副 9P 基材上搭 os-as-game 玩具。
- 其他：真 9P 寫入的 uid/權限、9P2000.L 擴充、把發布端接到真實 process 狀態（非示範 world）。
