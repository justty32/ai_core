...
- **結構決定①（升/降主線）**：確認 `core_handy`（C++）**升主線**、`sub_projs/ver_1/src/ai_core/_core.py`（Python）**降凍結參考**？此決定牽動哪套當地基往下做。
- **結構決定②（使用者自審地基）**：使用者將**自行審查 core_handy 是否合意的地基**。審查清單（都在 `../../sub_projs/ver_1/try_implement/core_handy/` 底下）：`examples/llm_entry.cpp`、`impl/serve.hpp`、`impl/llm.hpp`、`impl/rate.hpp`、`impl/http.hpp`、`defs/axes.hpp`、`CMakeLists.txt`。四個要回答的問題（詳見筆記 §9.6）：
