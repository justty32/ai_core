...
新增 / 修改 cl-lab 功能的工作流（改 `src/`、加依賴、擴 CLI、加測試/範例…）。這是本工作流的**入口**：先讀本檔，再往下深入。always-on 鐵律見 [AGENTS.md](../../AGENTS.md)；要整理結構時參考 [DEV-GUIDE](../../DEV-GUIDE.md)（被動）；**程式碼慣例 + 導航 index 維護鏈**見 [common/conventions](../common/conventions.md)。

## 流程

```
修改（增量）
  → 跑自動驗證（scripts/run.sh test；動到 build/entry 再跑 scripts/run.sh build）綠燈
  → 交使用者驗證（若需實機/實環境）→ 回報問題 → 修 → 重複
  → 全數通過後：對齊 INDEX 佈局表（兼 code map）→ 補對應 docs/examples → commit
```
