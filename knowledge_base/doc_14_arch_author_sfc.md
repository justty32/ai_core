# doc_14_arch_author_sfc

- **來源**：`docs/architectures/05_author_sfc.md`（§8 ai-core-author + §9 Small Function Center）
- **狀態**：舊版架構文件（部分已被 core_nature/roadmap 取代）
- **一行摘要**：另一版的 ai-core-author（generate→dry-run→register 一站式產生新函式，dry-run 強制）與 Small Function Center（SFC dispatcher 慣例）設計。
- **與現行差異提示**：
  - `ai-core-author` 對應 roadmap 的「authoring function / 函式自我增殖」概念；現行尚無此 CLI 二進位，但「上層聰明模型生資產、dry-run 驗證後才進 registry」的精神與 roadmap §第一目標問題一致，可信。
  - SFC（`ai-core-sfc`）對應 roadmap/CLAUDE.md 的「Small Function Center」概念與原型 `try_implement/tools/sfc.py`（現行原型已改接真 `ai_core`）。本版的 dispatch-flag 慣例、pass-through vs SFC 自管 metadata 等設計細節屬參考。
  - 「Calling Pack 就是普通獨立 function」的定位與現行「LLM Calling Packing」原型 `try_implement/lib/llm_call.py` 概念一致。
  - 路徑預設（`user_data_dir/ai_core/funcs/`、`platformdirs`）依賴外部相依，與現行純標準庫硬約束衝突；以現行為準時路徑機制僅為參考。
...
> 註：步驟 [5] 提及的 ledger 為此架構家族早期概念，後於 2026-05-12 整體移除 Calling Chain Observability / ledger（見 [doc_17_arch_changelog.md](doc_17_arch_changelog.md)）；此處流程圖保留原文。
...
`dispatch-flag` 的名稱（`--call`、`--run`、`--invoke`…）由實作者自訂；metadata 的 `usage` 欄位說明實際用法。I/O 慣例、stderr 錯誤、exit code 同全系統標準（[doc_11_arch_protocol.md](doc_11_arch_protocol.md) §5）。
...
hub scanner（[doc_13_arch_hub.md](doc_13_arch_hub.md) §7.5）遇到 SFC 時：
