...
> **狀態**：原型／探索（demo + 自我測試）。**測試結果已實機驗證**：72 + 68 = **140 項斷言全綠**（執行於 repo 的 `.venv` Python 3.14）。
> **一行摘要**：三個自我驗證的端到端 demo、兩套純標準庫煙霧測試（共 140 斷言）、範例函式與已 seed 的 SFC store。
...
```bash
python3 smoke_test.py        # 工具：72 斷言
python3 lib_smoke_test.py    # lib：68 斷言
...
## 兩套煙霧測試（140 斷言全綠，已實機確認）

### `smoke_test.py` — 工具端到端（72 斷言）
...
- **llm_entry_manager**（5）：metadata = persistent、宣告 singleton 資源、complete 回 text+usage、usage 查詢、超預算 → rate limit exceeded（consume rate 守門）。
- **chain**（4）：`--metadata` exit 0、跑管線 upper→reverse 得 `IH`、derive 複合 lifecycle=one_shot、derive 複合 guarantee=none（最弱）。

### `lib_smoke_test.py` — lib 各模組（68 斷言）
...
- **server**（7）：exit 0、ping→pong、list 含 handler、自訂 handler upper(hi)=HI、未知 cmd→ok=false、shutdown 回 shutdown=true、ready 訊號進 stderr。
...
- **llm_call**（3）：預設 EchoBackend、bind 疊 system 進 prompt、bind 疊 postprocess 後綴。
