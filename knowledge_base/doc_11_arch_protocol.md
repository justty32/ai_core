# doc_11_arch_protocol

- **來源**：`docs/architectures/02_protocol.md`（§4 metadata 協議 + §5 錯誤處理慣例）
- **狀態**：舊版架構文件（部分已被 core_nature/roadmap 取代）
- **一行摘要**：另一版的 metadata 協議——極簡 JSON 契約、慣例 key（`name`/`summary`/`io`/`examples`/`errors`/`dependencies`/`has_entries`/`entry_interface`/`server`）、容錯降級規則、`--json-errors` 旗標。
- **與現行差異提示（重要，本檔最易與現行九軸規範衝突）**：
  - **metadata 欄位體系完全不同**。本版以「描述類 + `io` + `examples` + `errors` + `dependencies`」為慣例 key，**沒有九軸概念**；現行 core_nature 用九軸（`entries`/`lifecycle`/`state`/`state_dirs`/`resources`/`interruptible`/`guarantee`/`dry_run`/`nondeterministic`）。兩套 schema 並存但**不相容**——以九軸（[doc_05_axes_metadata.md](doc_05_axes_metadata.md)）為現行事實。
  - 本版的 `io`（`input`/`output`/`format` 三態）對應現行九軸的 `entries`（`able_in`/`able_out`/`mode`/`type`/`channel_constraint`…，見 [doc_06_lib_contract.md](doc_06_lib_contract.md)）。設計目的相近，欄位形狀不同。
  - 本版**完全沒有第九軸 `nondeterministic`**，也沒有治理原則 / 證書概念。現行九軸中第九軸是治理錨點，見 [doc_05_axes_metadata.md](doc_05_axes_metadata.md) §9。本版可視為「八軸之前」更早的扁平 metadata 觀。
  - 本版的 `--metadata` 觸發機制描述為「程式接受旗標、直接輸出」，與現行 library 的「`register*` 純宣告 + `intercept()` 顯式攔截拆分」模型不同（見 [doc_06_lib_contract.md](doc_06_lib_contract.md)）。但「`<sub> --metadata`、pipe 透明、flag 不可在 `--metadata` 前」這些 CLI 規則與現行 [doc_07_cli.md](doc_07_cli.md) 高度一致，屬可共享的設計細節。
  - 本版的 `has_entries` / `entry_interface` / `--entry-metadata` 是此架構家族特有的 entrydata 介面（搭配 [doc_12_arch_entry_manager.md](doc_12_arch_entry_manager.md)），現行九軸體系無對應欄位。
...
metadata 在文字狀態下的表現形式是**標準 JSON**，且**鎖定 UTF-8 編碼**。任何以文字輸出的 metadata 都必須：
...
> ⚠️ 對應現行九軸的 `entries`（[doc_06_lib_contract.md](doc_06_lib_contract.md)）；欄位形狀不同。
...
**metadata 是了解一個工具的唯一入口**。但對於 entry-managing singleton（[doc_10_arch_overview.md](doc_10_arch_overview.md) §1），工具底下管理多個 entry；每個 entry 也有自己的資料介面，稱為 **entrydata**。
...
對需要常駐 server 才能運作的 wrapper（[doc_10_arch_overview.md](doc_10_arch_overview.md) §1 兩種亞型都適用），metadata 可宣告該 server 的典型啟用狀態與啟動方式，讓 hub 在清單中對 caller 顯示提示：
...
**這是純註解，不影響 wrapper 行為**：server 沒啟動時 wrapper 仍照 [doc_12_arch_entry_manager.md](doc_12_arch_entry_manager.md) §5.2 規則直接 stderr + exit 1。`activation` 與 `activation_hint` 只供 hub 顯示，幫助 caller 評估「呼叫這個 function 前要不要先啟動什麼」。
