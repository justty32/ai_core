...
- **來源**：`docs/architectures/07_agents.md`（§15 Agent 整合與文檔策略）
- **狀態**：舊版架構文件（部分已被 core_nature/roadmap 取代）
- **一行摘要**：另一版的 Agent 整合策略——文檔分層（手寫設計 vs 自動產生能力）、`AGENTS.md` 模板、hub `--export` 六種格式、自動文檔更新三觸發點、Self-Sufficiency 三階段演進路徑、與現成 agent 的接入 hint。
- **與現行差異提示**：
  - 此版以 `ai-core-hub --gen-*` / `--export` 自動產生 agent 文檔（AGENTS.md、FUNCTIONS.md、claude-skill、tools.openai/anthropic.json、server.mcp.json）。現行 Function Hub 規範未定（見 [doc_13_arch_hub.md](doc_13_arch_hub.md)），這些 export/gen 旗標屬參考設計。
  - Self-Sufficiency 三階段（Agent-assisted → Function-rich → Self-sufficient）與 roadmap 的「LLM 退縮到翻譯層 + 模糊前沿、其餘確定性碼」終點形狀概念相容，方向一致。
  - 本版提到的 `docs/EVOLUTION.md` 當時尚未建立。
