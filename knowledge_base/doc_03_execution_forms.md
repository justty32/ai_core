# doc_03_execution_forms

- **來源**：`core_nature/execution_forms.md`
- **狀態**：定案規範（部分待填）
- **一行摘要**：函式執行形式在 terminal 中的對應——§0 多維描述軸（含第九軸 nondeterministic）、§4 各執行形式定義（one-shot / multi-shot / persistent / singleton / streaming / lazy / detached / fan-out / idempotent / suspendable / wizard / transactional / memoized / dry-run / hook / ephemeral / proxy / agentic）、§5 觸發機制正交軸。

> 交叉引用：九軸詳規見 [doc_05_axes_metadata.md](doc_05_axes_metadata.md)、library 契約見 [doc_06_lib_contract.md](doc_06_lib_contract.md)、terminal 執行模型見 [doc_02_terminal_model.md](doc_02_terminal_model.md)、複合/組合規範見 [doc_08_composite.md](doc_08_composite.md)。
...
| **確定性 / 隨機性** | 同輸入是否同輸出；隨機環節（LLM）的認證狀態 | deterministic（預設）、nondeterministic（未認證 / 帶證書）；見 [`axis_spec.md §9`](../core_nature/axis_spec.md)（亦見 [doc_05_axes_metadata.md](doc_05_axes_metadata.md)） |

> 「確定性 / 隨機性」是繼前八軸之後**新增的第九軸**。前八軸隱含「函式是確定性的」前提，無一能描述
> LLM 這種天生隨機的函式，且此性質無法由任何既有軸隱含——故獨立成軸。它同時承載治理原則的證書
> （見 `roadmap.md §3.4`）。詳見 `axis_spec.md §9` 與 `lib_spec.md §9`（對應 [doc_05_axes_metadata.md](doc_05_axes_metadata.md) 與 [doc_06_lib_contract.md](doc_06_lib_contract.md)）。
