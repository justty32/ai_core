# doc_12_arch_entry_manager

- **來源**：`docs/architectures/03_entry_manager.md`（§6 LLM Entry Manager 完整設計）
- **狀態**：舊版架構文件（部分已被 core_nature/roadmap 取代）
- **一行摘要**：另一版的 LLM Entry Manager 完整設計——`ai-core-server`（HTTP）+ `ai-core-call`（wrapper CLI）雙元件，per-model asyncio queue、rate limit、time_to_wait 逾時、streaming、本機 token 認證。
- **與現行差異提示**：
  - 此版元件名 `ai-core-server` / `ai-core-call` 是「entry-managing singleton」首例，與 roadmap/CLAUDE.md 的「LLM Entry Manager 原型 `try_implement/tools/llm_entry_manager.py`」對應同一概念但**實作體系不同**：本版用 FastAPI + litellm + HTTP（外部相依），現行硬約束為純標準庫。以現行為準時，HTTP/asyncio/litellm 細節僅為參考設計。
  - 「LLM 是單例資源 → 佇列模式 → 集中管 consume rate」這個**核心概念與現行一致**（見 roadmap / [note_01_vision_and_origin.md](note_01_vision_and_origin.md)），可信。差異在於落地的協議與相依。
  - 本版的 `--entry-metadata` / entrydata 介面屬此架構家族特有，現行九軸體系無對應；見 [doc_11_arch_protocol.md](doc_11_arch_protocol.md) §4.7。

---

## §6. LLM Entry Manager 設計

> [doc_10_arch_overview.md](doc_10_arch_overview.md) §1「Singleton Resource Manager Pattern」的首個實例 — server (`ai-core-server`) + wrapper CLI (`ai-core-call`) 雙元件。
...
| 旗標 | 描述對象 | 內容 |
|---|---|---|
| `--metadata` | wrapper CLI 自身 | summary、usage、I/O 慣例（[doc_11_arch_protocol.md](doc_11_arch_protocol.md) §4） |
...
**`--entry-metadata` 內部 schema 也是自由的**（僅強制合法 JSON）。此介面屬 [doc_10_arch_overview.md](doc_10_arch_overview.md) §1 pattern 的一部分，未來其他 manager 也應提供同名旗標。
...
**`--output` 不污染原則**：`--output` 是 atomic 一次寫入（streaming 過程內容只在記憶體 buffer），失敗時直接不寫檔，與 [doc_11_arch_protocol.md](doc_11_arch_protocol.md) §5 規則一致。
...
**狀態管理由 caller 負責**（呼應 [doc_10_arch_overview.md](doc_10_arch_overview.md) §1 stateless 原則）：多輪對話、history、session 等概念**完全不存在於 wrapper 內**。caller 想做多輪對話 → 自己維護 `chat.json`、自己拼接訊息、自己更新檔案。wrapper 不記任何 session。
