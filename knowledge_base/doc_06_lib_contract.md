---
來源: core_nature/lib_spec.md（821 行，本批最長檔，已完整逐字覆蓋）
狀態: 定案規範（register/intercept 宣告攔截拆分為現行；§4/§9 自陳「從粗糙到嚴整」待 v0 收緊）
一行摘要: ai_core Python library 的 API 契約落地處——register/register_subcommand/register_subcommand_resolver/intercept 簽名與行為、--metadata 攔截規則、九軸 metadata fields 完整格式、memoized 不入軸決策。
---

> 軸的概念定義見 [doc_05_axes_metadata.md](doc_05_axes_metadata.md)；複合慣例見 [doc_08_composite.md](doc_08_composite.md)；正式核心實作狀態見 [code_01_core_library.md](code_01_core_library.md)；決策脈絡見 [note_06_decisions_and_open_questions.md](note_06_decisions_and_open_questions.md)。
