---
來源: core_nature/composite_spec.md（263 行）
狀態: 定案規範（含「並發不處理」「nondeterministic 複合慣例候選」等待議標記）
一行摘要: 複合式規範（composite conventions）——橫跨多軸的具名慣例：標準狀態目錄慣例（.config/.cache/.state/.data）與中斷恢復慣例（recovery.json）。
---

> 各軸的原子 metadata fields 定義於 [doc_06_lib_contract.md](doc_06_lib_contract.md)；軸的概念定義見 [doc_05_axes_metadata.md](doc_05_axes_metadata.md)；nondeterministic 複合慣例候選的脈絡見 [note_06_decisions_and_open_questions.md](note_06_decisions_and_open_questions.md)。
