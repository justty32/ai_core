# doc_17_arch_changelog

- **來源**：`docs/architectures/08_changelog.md`（§16 變更紀錄）
- **狀態**：舊版架構文件（部分已被 core_nature/roadmap 取代）
- **一行摘要**：此舊版架構家族（`docs/architectures/`）自身的演化變更紀錄，2026-04-28 初稿到 2026-05-18，記錄 Singleton Pattern 抽象、Ledger 引入後移除、A/B/C 類缺口處理、protocol/ helper 宣告等里程。
- **與現行差異提示**：
  - 這份 changelog 記的是「舊版架構文件自己怎麼演化」，**不是現行 core_nature/roadmap 的變更史**。內含若干已被自我推翻的概念（如 Calling Chain Observability / ledger / `ai-core-trace`，於 2026-05-12 整體移除）。
  - 此版自始至終未出現「九軸」「`nondeterministic` 第九軸」「`register()`/`intercept()`」等現行概念——印證本架構家族屬於另一條設計支線，而非現行九軸/library 體系的前身。現行變更史以 roadmap 與 `try_implement/DECISIONS.md` 為準。
  - 紀錄中提及的 `docs/OPEN_QUESTIONS.md` 已於 2026-05-12 刪除。
