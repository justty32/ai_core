...
原本這兩個動作的 LLM 加工是 shell out 給 ai_core 自己的 `idea.py`（dogfood 自家 LLM 基礎設施，經元件 1 entry manager 路由、宣告第九軸 `nondeterministic:true`）。該工具已隨早期實作封存（半封存狀態），當前無在跑的實作、要用把它當參考。
