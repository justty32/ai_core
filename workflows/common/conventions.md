...
4. **Function Hub** — 掃描函式集、呼叫各 `--metadata`、彙整成「給 LLM 的 skill 清單」，含 context budget 逐級收斂。原型：`try_implement/tools/hub.py`（規範未定）。
5. **Small Function Center (SFC)** — 把大量 tiny function 集中到 git-style subcommand dispatcher，避免檔案爆炸。原型：`try_implement/tools/sfc.py`（已接真 `ai_core`）。

> **點子捕捉軌 dogfood**：`try_implement/tools/idea.py` 串起 `bind`（元件2）→ entry manager（元件1）→ 真 backend → API，是「廉價小模型消費者」的第一個真實串接。詳見 [idea-capture 工作流](../idea-capture.md)。
