...
| [sub_projs/](sub_projs/README.md) | 次級物：`llm_forge/`（框架規劃地／爐子：固化階梯等機制之家）＋ `galtxt/`（動手實驗場／第一把刀：依賴 llm_forge，先跑通再搬回固化）＋ `cllm/`（獨立 C LLM 產物：對外 C ABI `libcllm.so`＋`llm` unix filter CLI，源自 galtxt/try_3 抽離）＋ `comfy/`（舒適 Common Lisp 地基＋VSCode/Alive debug 環境，框架外實驗場）＋ `ver_1/`（早期 Python 實作封存，半封存、不在現役維護鏈）。入口 [sub_projs/README.md](sub_projs/README.md) |
