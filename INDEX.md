...
| [sub_projs/](sub_projs/README.md) | 次級物（**2026-07-20 結構重整**）：**`cllm/` 傘**（`core/`＝獨立 C LLM 產物：對外 C ABI `libcllm.so`／`.dll`＋`llm` CLI，源自 galtxt/try_3 抽離；`apps/`＝原 cllm-apps，handy 四工具多語言移植；`lab/`＝原 cllm-lab，綁定十語言遊樂場）＋ **`lang-labs/` 傘**（`cl-lab/`＝Common Lisp 遊樂場，內含 `comfy/` 舒適 CL 地基；`janet-lab/`＝Janet 遊樂場，unipath 規則語言學習底本）＋ `galtxt/`（動手實驗場／第一把刀，〔暫緩〕）＋ `handy/`（路徑一試驗田／OS 當 AI agent，**2026-07-21 重構完成**：地基＝`util/llm`＝litellm 薄包裝，舊 `pllm` 與 Fennel 版住戶均已封存）＋ `unipath/`（歸一於路徑基材：Plan 9＋9P over FUSE，step 1–6 全綠；Windows 跑不了）＋ `llm_forge/`（框架規劃地／爐子）＋ `ver_1/`（早期 Python 封存）。入口 [sub_projs/README.md](sub_projs/README.md) |
