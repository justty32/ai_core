...
- **審議 crystallization_engine_blueprint.md（spec 候選）是否經 `/spec` 扶正** — 固化引擎 v0 工程藍圖已具函式簽章／編排流程／里程碑，標 `stage: spec-candidate`；待使用者裁決是否收斂進規範，關聯 [try_implement/DECISIONS.md](try_implement/DECISIONS.md) A4（組合軸推導）與 [core_nature/composite_spec.md](core_nature/composite_spec.md)。見 [ideas/crystallization_engine_blueprint.md](ideas/crystallization_engine_blueprint.md)。
