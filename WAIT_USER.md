...
- **在 VSCode 裡確認 comfy 語法糖的高亮**：注入文法擴充已備於 [sub_projs/comfy/editor/vscode-comfy/](sub_projs/comfy/editor/vscode-comfy/README.md)（我這邊無法渲染驗證）。啟用（`F5` 開發宿主，或複製到 `%USERPROFILE%\.vscode\extensions\` 重啟）後看 `true`/`false`／`'a'`／字串 `\n` 有沒有正確上色；沒生效就用「Developer: Inspect Editor Tokens and Scopes」查真 scope 名回報，我再調文法。
