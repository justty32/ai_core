# doc_02_terminal_model

- **來源**：`core_nature/terminal_model.md`
- **狀態**：定案規範（部分待填）
- **一行摘要**：從 terminal 出發的執行模型基礎——POSIX shell 原生概念（命令解析、CLI 慣例、subcommand 模式），本規範以此為基礎並大致依循。

> 交叉引用：九軸詳規見 [doc_05_axes_metadata.md](doc_05_axes_metadata.md)、library 契約見 [doc_06_lib_contract.md](doc_06_lib_contract.md)、本質定位見 [doc_01_nature_overview.md](doc_01_nature_overview.md)、CLI 介面契約見 [doc_07_cli.md](doc_07_cli.md)。
...
這種模式的支援僅由程式撰寫者自行實現，本規範只是規定說，若有類似的 subcommand 操作需要實現，那就必須用這種表現方式。
