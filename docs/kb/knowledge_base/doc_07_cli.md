---
來源: core_nature/cli_spec.md（75 行）
狀態: 定案規範
一行摘要: CLI 介面契約——CLI 呼叫即 Lisp 風格函式呼叫（keyword pair → dict / positionals → array）、argparse 解析、本規範取捨與 --metadata 新增項。
---

> 軸定義見 [doc_05_axes_metadata.md](doc_05_axes_metadata.md)；library 契約（含 --metadata 攔截）見 [doc_06_lib_contract.md](doc_06_lib_contract.md)；正式核心實作狀態見 [code_01_core_library.md](code_01_core_library.md)。
