# doc_15_arch_project_milestones

- **來源**：`docs/architectures/06_project.md`（§10 目錄結構 + §10.1 protocol/ + §11 CLI + §12 config + §13 里程碑 M0–M6 + §14 跨平台）
- **狀態**：舊版架構文件（部分已被 core_nature/roadmap 取代）
- **一行摘要**：另一版的工程藍圖——`src/ai_core/` 五元件目錄樹、`protocol/` 共用 helper 出口、`[project.scripts]` 六個 CLI 入口、`config.json` 範例、開發里程碑 M0–M6、跨平台七要點。
- **與現行差異提示**：
  - 此版目錄樹（`entry_manager/`、`client/`、`hub/`、`small_funcs/`、`author/`、`protocol/`）與現行**完全不同**：現行正式核心只有單檔 `src/ai_core/_core.py`（＋ `tests/test_core.py`），沒有這些子套件。以 CLAUDE.md「目錄結構」一節為現行事實。
  - 此版 config / CLI 大量依賴 `platformdirs`、`fastapi`、`litellm`、`uv`，**違反現行純標準庫硬約束**。`config.json` 範例、port 號（5577/5578）、`[project.scripts]` 六入口皆為舊版提案。
  - 此版里程碑 M0–M6 與 roadmap §6 的「v0 最小垂直切片」**不是同一套規劃**；現行以 roadmap 的目標問題驅動法為準，本里程碑僅作參考。
  - 跨平台一節涵蓋 Windows；現行明定「Windows 不在考慮範圍，目標平台 POSIX」。本版的 Windows 處理（msvcrt、ACL）與現行方向相左。
...
| 子模組 | 已存在 | 內容 | 主要服務對象 |
|---|---|---|---|
| `metadata.py` | ✅ | `fetch_metadata(path) -> MetadataView`、`make_json_error` / `print_json_error` / `should_use_json_errors`（[doc_11_arch_protocol.md](doc_11_arch_protocol.md) §4.6 / §5.1） | 所有 function、hub、caller |
| `env.py` | ✅ | 全系統共用環境變數名稱常數（`AI_CORE_TOKEN` / `AI_CORE_FUNCS_DIR` / …） | 所有元件 |
| `server.py` | ⏳ 規劃 | Singleton Resource Manager Pattern 的 server 樣板：token 產生 / 讀檔（[doc_12_arch_entry_manager.md](doc_12_arch_entry_manager.md) §6.13）、FastAPI app 工廠、graceful shutdown、`--metadata` / `--entry-metadata` 端點骨架 | 任何 server-backed function、第二個以後的 singleton manager |
...
- 所有 helper 都允許 graceful 降級（缺欄位用合理預設、不丟例外給 caller），與 [doc_11_arch_protocol.md](doc_11_arch_protocol.md) §4.6 / §5 的容錯原則一致
...
> ⚠️ **與現行衝突**：`platformdirs` 為外部相依，違反現行純標準庫硬約束。port、config schema 皆為舊版提案。
...
> ⚠️ **與現行衝突**：此 M0–M6 與 roadmap §6「v0 最小垂直切片」不是同一套規劃；以 roadmap 為現行事實。
...
> ⚠️ **與現行衝突**：現行目標平台為 POSIX，Windows 不在考慮範圍。下表 Windows 相關處理（msvcrt、ACL、named pipe）為舊版才有的考量。
