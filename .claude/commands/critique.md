...
---
進入「找漏洞模式」（WF4），對 `workflows/ideas/` 軌的點子做嚴格但善意的審查。審查的實際加工**不由你（agent）自己做，改由 ai_core 的 LLM 工具 `idea critique` 打 API 完成**（dogfood：`bind` → LLM Entry Manager → 真 backend）。全程繁體中文。
