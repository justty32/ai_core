---
description: 規範扶正 — 把 try_implement 提案收斂進 docs/spec/ 規範與 _core.py
...
- 「扶正」＝把某提案落地進**正式核心**：`src/ai_core/_core.py`（API／九軸）與 `docs/spec/`（spec 文件），並補 `tests/test_core.py`。
- 設計哲學是硬約束：**KISS／輕量／不重造輪子／相依最少**；實作**只用 Python 3.11+ 標準庫**，`pyproject.toml` 維持 `dependencies = []`；目標平台 POSIX。

## 流程
1. 讀 `DECISIONS.md` 對應條目（現況／建議／待決點）與相關 `docs/spec/*.md`。
2. **拒絕為預設**：除非該環節通過「必要性＋穩定性」兩道閘門，否則不擴張規範（呼應第九軸治理原則）。先選目標問題、讓它告訴你該定哪條——**目標問題＝停止鍵**。
3. 扶正落地：改 `_core.py` ／ 寫 `docs/spec/` ／ 補 `tests/test_core.py`，跑 `/test` 驗證全綠。
