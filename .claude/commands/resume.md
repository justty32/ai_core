...
2. 再依當下焦點擇一：
   - 懸案 → `sub_projs/ver_1/try_implement/DECISIONS.md`（待定奪的規範決策清單）。
   - 規範現況 → `workflows/spec/composite_spec.md`（及 `workflows/spec/` 其餘 spec）。
...
| 規範 (spec) | `workflows/spec/` | 已扶正＝定案；標「待填／待議」＝未定 |
| 原型 (探索) | `sub_projs/ver_1/try_implement/` | 多為提案，非定案；扶正狀態見其 `DECISIONS.md` |

正式核心程式碼只有一處：`sub_projs/ver_1/src/ai_core/_core.py`（＋ `sub_projs/ver_1/tests/test_core.py`）。
