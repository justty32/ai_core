...
> 早期的 `thinking*.md`（`thinking.md`、`thinking_layers.md`、`thinking_oop.md` …）是歷史思考留存，已被 `roadmap.md` 與 `core_nature/` 取代為當前事實來源；讀它們是為了理解演化脈絡，不是為了取得現況。
...
│   ├── demos/              #   3 個可跑 demo
│   ├── smoke_test.py       #   80 項斷言
│   └── lib_smoke_test.py   #   76 項斷言
├── funcs/                  # 範例函式（echo.sh …）
...
.venv/bin/python try_implement/smoke_test.py      # 80 項斷言
.venv/bin/python try_implement/lib_smoke_test.py  # 76 項斷言
